...
def initialize_pathfinder():
    ...
    def pathfinderThread():
        ...
        global iterastar, pathpoints, newtarget, start, finish
        if on_main_thread:
            on_main_thread.pop()
            return
        # stuff happens
...
def astar():
    ...
    while openset:
        ...
        for i in neighbors(current[1]):
            ...
            if i in closedset:
                continue
            if i in f_score and f_score[i] <= tentative_f:
                continue
            if i not in seen or tentative_f < f_score[i]:
...
def trace_path(src, dest):
    ...
    """
    #print 'trace_path', locals(), camefrom
    path = [dest]
    cur = camefrom[dest]
    while cur != src:
        #print cur
